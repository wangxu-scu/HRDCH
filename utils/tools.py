...
def get_clean_and_noisy_index(dataset, noise_rate):
    if dataset == "nuswide21":
        ...
        noise = h5py.File(
            "./noise_label/IAPR-lall-noise_{}.h5".format(noise_rate)
        )
    fl = list(noise["True"])
...
def SaveH5File_I(resize_size):
    ...
    test_labels = data["testL"]
        ...
        np.concatenate([valid_img, test_img]),
        np.concatenate([valid_txt, test_txt]),
        np.concatenate([valid_labels, test_labels]),
    )
    query_size = 2000
    train_size = 10000
    imgs = list(fi[query_size : query_size + train_size])
...
def compute_img_result(dataloader, net, device):
    ...
    for img, tag, tcls, cls, _ in tqdm(dataloader):
        ...
        view1_feature, view2_feature,_,_ = net(img.to(device), tag.to(device))
        bs.append(view1_feature.data.cpu())
...
def compute_tag_result(dataloader, net, device):
    ...
    for img, tag, tcls, cls, _ in tqdm(dataloader):
        ...
        tag = tag.float()
        view1_feature, view2_feature,_,_ = net(img.to(device), tag.to(device))
        bs.append(view2_feature.data.cpu())
    return torch.cat(bs).sign(), torch.cat(clses)
...
def calc_map_k(rB, qB, retrieval_label, query_label, k=None,device=torch.device("cuda:0")):
    # qB: {-1,+1}^{mxq}
    # rB: {-1,+1}^{nxq}
    # sim: {0, 1}^{mxn}
    ...
    map = 0.0
    GND = (query_label.mm(retrieval_label.t()) > 0).type(torch.float).squeeze().to(device)
    if k is None:
...
def plot_gmm(gmm, X, clean_index, noisy_index, save_path="", plot_pdf=True):
    ...
    if plot_pdf:
        ...
        ax.plot(
            x,
            pdf_individual[:, gmm.means_.argmin()],
            "--",
            label="Component A",
            color="green",
        )
        ax.plot(
            x,
            pdf_individual[:, gmm.means_.argmax()],
            "--",
            label="Component B",
            color="red",
        )
